...
describe('AuthController', () => {
  ...
  it('refresh sets a rotated cookie and strips the refresh token from the body', async () => {
    const user = { ...baseUser, jti: 'session-1' };
    authService.refresh.mockResolvedValue({
      message: 'Token refreshed successfully.',
      data: { accessToken: 'new-access', refreshToken: 'new-refresh' },
    });

    const result = await controller.refresh(user, response as unknown as Response);
  ...
  it('logout reads the cookie, clears it, and returns the service result', async () => {
    ...
    const result = await controller.logout(baseUser, req, response as unknown as Response);
  ...
  it('me returns the authenticated user injected by @CurrentUser()', () => {
    const result = controller.me(baseUser);
